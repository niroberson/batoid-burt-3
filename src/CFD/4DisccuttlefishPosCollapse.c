...
versus a very large file) */
	...
	real NV_VEC(origin), NV_VEC(rvec), NV_VEC(pt1), NV_VEC(pt2), NV_VEC(pt3);
	real NV_VEC(pt4), NV_VEC(pt5), NV_VEC(opt1), NV_VEC(opt2), NV_VEC(opt3);
	real NV_VEC(opt4), NV_VEC(opt5);
	double div1 = -0.2;
	double div2 = -0.1;
	double div3 = 0.0;
	double div4 = 0.1;
	...
	begin_f_loop(f,tf) /* Loop over every face, f in thread, tf */
	{
		...
		/* For every face, define the integer value of every node composing the face */
		{
			...
			if(NODE_POS_NEED_UPDATE(v))
			{
				...
				pt3[1] = (div3+0.2)*0.25*sin(2*M_PI/0.335*div3-15*time)*(1-exp(-time));
				pt4[1] = (div4+0.2)*0.25*sin(2*M_PI/0.335*div4-15*time)*(1-exp(-time));			
				pt5[1] = (div5+0.2)*0.25*sin(2*M_PI/0.335*div5-15*time)*(1-exp(-time));			
				
				opt1[1] = (div1+0.2)*0.25*sin(2*M_PI/0.335*div1-15*(time-dtime))*(1-exp(-(time-dtime)));
				opt2[1] = (div2+0.2)*0.25*sin(2*M_PI/0.335*div2-15*(time-dtime))*(1-exp(-(time-dtime)));	
				opt3[1] = (div3+0.2)*0.25*sin(2*M_PI/0.335*div3-15*(time-dtime))*(1-exp(-(time-dtime)));
				opt4[1] = (div4+0.2)*0.25*sin(2*M_PI/0.335*div4-15*(time-dtime))*(1-exp(-(time-dtime)));	
				opt5[1] = (div5+0.2)*0.25*sin(2*M_PI/0.335*div5-15*(time-dtime))*(1-exp(-(time-dtime)));				
				
				if(NODE_X(v)<div2) {
				posNew[1] = (pt2[1]-pt1[1])/(div2-div1)*(NODE_X(v)-div1)+pt1[1];
				}
		
				else if (NODE_X(v)>div2 && NODE_X(v)<div3) {
				posNew[1] = (pt3[1]-pt2[1])/(div3-div2)*(NODE_X(v)-div2)+pt2[1];
				}
				
				else if (NODE_X(v)>div3 && NODE_X(v)<div4) {
				posNew[1] = (pt4[1]-pt3[1])/(div4-div3)*(NODE_X(v)-div3)+pt3[1];
				}
				
				else if (NODE_X(v)>div4) {
				posNew[1] = (pt5[1]-pt4[1])/(div5-div4)*(NODE_X(v)-div4)+pt4[1];
				}
				
				if(time > dtime) {
			
				if(NODE_X(v)<div2) {
				posOld[1] = (opt2[1]-opt1[1])/(div2-div1)*(NODE_X(v)-div1)+opt1[1];
				}
				
				else if (NODE_X(v)>div2 && NODE_X(v)<div3) {
				posOld[1] = (opt3[1]-opt2[1])/(div3-div2)*(NODE_X(v)-div2)+opt2[1];
				}
				
				else if (NODE_X(v)>div3 && NODE_X(v)<div4) {
				posOld[1] = (opt4[1]-opt3[1])/(div4-div3)*(NODE_X(v)-div3)+opt3[1];
				}				
				else if (NODE_X(v)>div4) {
				posOld[1] = (opt5[1]-opt4[1])/(div5-div4)*(NODE_X(v)-div4)+opt4[1];
				}
